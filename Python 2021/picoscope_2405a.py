...
# analogOffset = 0 V
chDRange = 8
status["setChD"] = ps.ps2000aSetChannel(chandle, 3, 1, 1, chDRange, 0)
...
# analogOffset = 0 V
chERange = 7
status["setChE"] = ps.ps2000aSetChannel(chandle, 4, 0, 1, chERange, 0)
...
# analogOffset = 0 V
chFRange = 7
status["setChF"] = ps.ps2000aSetChannel(chandle, 5, 0, 1, chFRange, 0)
...
# analogOffset = 0 V
chGRange = 7
status["setChG"] = ps.ps2000aSetChannel(chandle, 6, 0, 1, chGRange, 0)
...
# analogOffset = 0 V
chHRange = 7
status["setChH"] = ps.ps2000aSetChannel(chandle, 7, 0, 1, chHRange, 0)
...
status["trigger"] = ps.ps2000aSetSimpleTrigger(chandle, 1, 1, 8200, 2, 0,00) #This sets a trigger level of 1.25V. This WILL negelect -1.25 V. Needs optimization
...
status["getTimebase2"] = ps.ps2000aGetTimebase2(chandle, timebase, maxSamples, ctypes.byref(timeIntervalns), ctypes.byref(returnedMaxSamples), 0)
...
# pParameter = None
status["runBlock"] = ps.ps2000aRunBlock(chandle, preTriggerSamples, postTriggerSamples, timebase, None, 0, None, None)
...
status["setDataBuffersC"] = ps.ps2000aSetDataBuffers(chandle, 2, ctypes.byref(bufferCMax), ctypes.byref(bufferCMin), maxSamples, 0 , 0)
assert_pico_ok(status["setDataBuffersC"])

status["setDataBuffersD"] = ps.ps2000aSetDataBuffers(chandle, 3, ctypes.byref(bufferDMax), ctypes.byref(bufferDMin), maxSamples, 0 , 0)
...
axs[0].plot(time, adc2mVChAMax[:],"r-")
axs[1].plot(time, adc2mVChBMax[:],"b-")
axs[2].plot(time, adc2mVChCMax[:],"o-")
axs[3].plot(time, adc2mVChDMax[:],"g-")
axs[0].title.set_text("Channel A")
axs[1].title.set_text("Channel B")
axs[2].title.set_text("Channel C")
axs[3].title.set_text("Channel D")
plt.xlabel('Time (ns)')
...
with open(file_name,'a') as f:
    ...
    writer.writerow(adc2mVChCMax[:])
    writer.writerow(adc2mVChDMax[:])
